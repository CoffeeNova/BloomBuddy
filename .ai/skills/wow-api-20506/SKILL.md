...
| `OptionsSliderTemplate` | Does **NOT** create a global `"<name>Text"` (unlike `UICheckButtonTemplate`) | create the label yourself: `parent:CreateFontString(...)`; it DOES create `<name>Low`/`<name>High` (LoseControltbc_anni sets them via `_G[name .. "Low"]`). `OnValueChanged(self, value)` fires on `SetValue` too — guard with a `_syncing` flag |
| `UIPanelDialogTemplate` | Works on this client (OmniCC_Config `preview.lua:19`); children use `$parent`-based names (e.g. `$parentTitleBG`, **`$parentCloseButton`**, `$parentText`) | **NAME the frame** (`CreateFrame("Frame", "MyOptionsFrame", UIParent, "UIPanelDialogTemplate")`) — an unnamed frame risks bad/empty globals; **reuse the built-in `$parentCloseButton`** (set `OnClick` → `Hide()`) — creating a second close button yields two overlapping X buttons (verified by user report 2026-08-16); anchor the title to `$parentTitleBG`'s CENTER |
| `UICheckButtonTemplate` / `UIPanelCloseButton` / `UIPanelButtonTemplate` | Need an explicit `OnClick` (close button: `Leatrix_Maps`); unnamed checkboxes are safe (AtlasLootClassic) with manual label FontStrings; `SetChecked` does NOT fire `OnClick` | set `SetScript("OnClick", ...)` yourself; label = `parent:CreateFontString` anchored `LEFT` of the check |
