...
2. **Read repo memory**: `/memories/repo/bloom-buddy.md` + topic files (gotchas, decisions, phases) — they carry the verified history; don't re-learn what's there.
...
8. **Update memory**: append a `Phase N DONE` line to `/memories/repo/`; add NEW gotchas to `bloom-buddy-gotchas.md` and the `wow-api-20506`/`unit-frame-buffs` skills; update decisions/status files.
...
- **Automated unit tests do not exist yet** (planned for Phase 3 — `.github/docs/addon-v1-development-plan.md`). Until then, verification is manual.
...
Always check the phase's Definition of Done in `.github/docs/addon-v1-development-plan.md`. If you can't verify in-game yourself, state clearly what the user must verify.
