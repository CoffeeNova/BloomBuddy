...
Everything an agent needs — project context, architecture, as well as skills, agents and instructions — lives in the **`.agents/`** directory. It is the single source of truth.

## Reading order

1. `.agents/CONTEXT.md` — what the addon does, code conventions, key mechanics & gotchas, proven patterns, settings reference
2. `.agents/ARCHITECTURE.md` — module design, data flow, edge cases, ADR decisions
3. `.agents/skills/wow-api-20506/SKILL.md` — **before writing/fixing any WoW API call**: verified client gotchas & patterns (many APIs crash or return shifted data on 2.5.5)
4. `.agents/skills/unit-frame-buffs/SKILL.md` — **before touching the core feature**: verified/researched patterns for enlarging buff icons on party and raid frames

## Rules

- Follow everything in `.agents/`. It is the source of truth.
- When the design or plan changes, update the files in `.agents/` **first** — they are the contract.
...
- **Contract-first, memory-last**: update `.agents/` before code; record outcomes in `/.agents/memories/repo/` after.
- **Static analysis**: every Lua file is analyzed for **errors and warnings** with the **luahelper MCP** server before it is considered done (see `.agents/CONTEXT.md`).
...
- Fix all real errors/warnings. Informational annotation warnings (type 18, `---@class`/`---@type` underlines) are expected workspace noise in this repo — do not churn code to silence them (see `.agents/CONTEXT.md`).
...
- The suite does NOT exist yet — it is Phase 3 of the development plan (`.agents/docs/addon-v1-development-plan.md`).
- Before writing or running tests, read the `unit-testing` skill (`.agents/skills/unit-testing/SKILL.md`).

## Why this repo looks the way it does

This repository is a lean, adapted port of the development toolkit proven in the sibling addon **ArenaChillPrep** (same client, same module conventions). The `.agents/` library, the `Data/Utils/Classes` split, the `BB` vararg chain and the tooling are intentionally shared patterns — keep them consistent when you extend this addon.
